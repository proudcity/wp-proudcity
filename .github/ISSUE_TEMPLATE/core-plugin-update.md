...
- **the live Stateless verification has been done and it passed** — 2026-08-21 on beta, several folders uploaded through the "Upload folder" button. All eight attachments carry non-empty `sm_cloud` (bucket `proudcity`) and resolve to `storage.googleapis.com`, including PDF and DOCX, and every stored filename carries the `-[8 hex]` suffix from our `stateless_skip_cache_busting` filter. Run against the pre-release build `2887.2026.08.21.1019`, which is identical to 6.2.7 on the upload path. **Do not re-run this as routine per-release work** — repeat it only if the regression check above finds a defect has come back
- how to repeat it if needed: upload a PDF through the "Upload folder" button on a Stateless site, then confirm the attachment has non-empty `sm_cloud` post meta and that `wp_get_attachment_url()` returns a `storage.googleapis.com` URL. A `/wp-content/uploads/` URL means the bug is back
### WP-Stateless
